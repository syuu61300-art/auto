-- AutoClicker 本体 (GitHubにアップするファイル)
-- ファイル名: AutoClicker.lua
...
local Window = Rayfield:CreateWindow({
    ...
    LoadingSubtitle = "Powered by Syu_hub",
    ConfigurationSaving = {
        Enabled = false,
    },
    Discord = {
...
local CPSLabel = InfoTab:CreateLabel("間隔: 1ms (最大1000 CPS)")
local SessionLabel = InfoTab:CreateLabel("経過時間: 0秒")

-- セクション
MainTab:CreateSection("オートクリック")
...
local Toggle = MainTab:CreateToggle({
    ...
    Callback = function(value)
        ...
        if isClicking then
            ...
            Rayfield:Notify({
                Title = "AutoClicker",
                Content = "クリック停止　合計: " .. tostring(clickCount) .. "回",
                Duration = 3,
                Image = 4483362458,
            })
        end
